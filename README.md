...
Windows Toolbox 是一个面向 Windows 10/11 x64 的原生系统工具集合。V0.2.0 提供文件、端口、进程和基础网络诊断，并从一开始采用可扩展的 Tool Host + Tool Module 架构。
...
- 进程关系：显示进程映像路径、启动时间、完整父进程链和直接子进程，支持完整报告、刷新与按 PID 跳转端口页。
...
- V0.2.0 不使用未公开 NT API，因此不读取任意进程命令行，也不实现深度句柄扫描、强制关闭 Handle、驱动、注入或权限绕过。
